...
<body>
        ...
        async function initMap() {
            ...
            async function geocode_address(position, marker) {
                try {
                    ...
                    const url = \`https://maps.googleapis.com/maps/api/geocode/json?latlng=\${plainPosition.lat},\${plainPosition.lng}&key=AIzaSyCpKLgIGARppYtXkaBsneFZVwG1f6oPNWo\`;
                    ...
                    marker.address = 'Geocoding failed';
                }
            }
            ...
            function updateAllMarkerData() {
                ...
                draggableMarkers.forEach((marker, index) => {
                    if (marker && marker.position) {
                        ...
                        latitudes.push(lat);
                        longitudes.push(lng);
                        addresses.push(marker.address || 'Address pending...');
    ...
    $(document).on('ready', async function () {
        ...
        try {
            ...
            const [latInput, lngInput, zoomInput, addressInput, buttonInput] = await Promise.all([
            ...
            let initAddresses = parseAddresses(addressInput.attr('value'), ["default"]);
            ...
            window.addEventListener('message', async function (event) {
                ...
                if (event.data.type === 'mapUpdate') {
                    ...
                    if (data.addresses != ["default"]) {
                        $("#buttons\\.view_proof").text("Continue");
                        $("#buttons\\.view_proof").parent().prop("disabled", false);
                    }
                    
                    console.log(data.addresses.join('~'));
            ...
            $("#buttons\\.view_proof").text("Please adjust the map marker.");
            $("#buttons\\.view_proof").parent().prop("disabled", true);
