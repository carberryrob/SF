...
if cnt < 4:   ### If 3 or less addresses are selected then we leave the back blank
	COND(COND_EMPTY)
elif cnt >= 4:
	...
	if (cnt < 6):   ### Standard layout for 5 or less addresses
		vs = 5.3
	...
	else:   ### Layout for 6 addresses we make the vertical spacing between addresses smaller, and we remove spacing between the address and phone numbers. 
		vs = 4.3
		if card_address != '-None':
			add1 = DWIMLINES([LEFT, F11, 'q  ', F1, COL, card_street1, NEWLINE,
			COL, card_street2, NEWLINE,
